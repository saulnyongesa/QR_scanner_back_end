...
def generate_qr_buygoods(request):
    if request.method == "POST":
        ...
        try:
            data = QRCodeForBuyGoods.objects.filter(till_number=till_number)
            if data:
            ...
            messages.success(request, 'QR Code for Till Number ' + till_number + " generated successfully")
            return redirect('show-qr-buygoods-url')
...
def generate_qr_paybill(request):
    if request.method == "POST":
        ...
        try:
            data = QRCodeForPayBill.objects.filter(business_number=business_number)
            if data:
            ...
            messages.success(request,
                             'QR Code for Business Number' + business_number + " and Account No " + account_number + " generated successfully")
...
def sign_up(request):
    if request.method == "POST":
        ...
        if password != confirm_password:
            ...
            messages.error(request, "Password less than 8 digits")
            return redirect('sign-up-url')
        else:
            try:
                em = User.objects.filter(email=email)
                phone1 = User.objects.filter(phone_number=phone)
                if em:
